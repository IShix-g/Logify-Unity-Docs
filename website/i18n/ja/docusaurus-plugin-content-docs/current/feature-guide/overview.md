...
**Logify** は、Unityモバイルゲーム開発における「実機デバッグの煩わしさ」を解消するために設計された、高機能ランタイム・デバッグツールです。

### ✨ 主な機能

Logifyは、実機デバッグに必要なツールを統合した、以下の5つの主要機能を備えています。
...
インポート後の**初期セットアップは不要**です。シーンにプレハブを置いたり、初期化コードを記述する必要はありません。

<img src={require('./img/trigger.png').default} width="420" alt="トリガー位置の確認" />

任意のシーンでPlayモードを開始し、**画面の左上隅をロングプレス**することで、デバッグ用ダイアログが立ち上がります。

:::note Tip
* 自動初期化を無効にし、任意のタイミングで**手動初期化**を行うことも可能です
* トリガーの位置や長押しの判定時間は、プロジェクトに合わせて自由にカスタマイズできます

これらの設定は **Window > Logify > Settings** から変更可能です。
:::
